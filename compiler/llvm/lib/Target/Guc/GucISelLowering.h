//==-- GucISelLowering.h - Guc DAG Lowering Interface ------*- C++ -*-==//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines the interfaces that Guc uses to lower LLVM code into a
// selection DAG.
//
//===----------------------------------------------------------------------===//
...
namespace llvm {
  namespace GucISD {
    enum {
      ...
      /// Guc conditional branches. Operand 0 is the chain operand, operand 1
  ...
  class GucTargetLowering : public TargetLowering {
  public:
    explicit GucTargetLowering(GucTargetMachine &TM);
    ...
    /// Ty1 to type Ty2. e.g. On guc it's free to truncate a i32 value in
    ...
    /// necessarily apply to truncate instructions. e.g. on guc, all
